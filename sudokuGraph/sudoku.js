...
function getInitialHelp() {
  ...
  if (naked_single) {
    ...
    let c = document.querySelector(
      `#cell${naked_single[0][0].toString()}${naked_single[0][1].toString()}`
    );
    c.focus();
  ...
  document.querySelectorAll(".bigN").forEach((el) => {
    ...
    el.addEventListener("keydown", (evt) => {
      ...
      if ((evt.key === "Backspace" || evt.key === "Delete") && el.value == "") {
        ...
        // hard delete the input because sometime the backspace doesn't work ---- TODO - get all instances of backspace stuff
...
window.addEventListener("load", () => {
  ...
  document.querySelector("#instructionBtn").addEventListener("click", () => {
    ...
    if (toggleInstructions) {
      //console.log(document.querySelector("instructions").classList);
    ...
    // toggle if instructions are hidden
